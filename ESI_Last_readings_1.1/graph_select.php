...
//Gráfica valores mínimo, máximo y medio
if ($_GET['grafica'] == 2){
	
	$columna = $_GET['valor'];
	$consulta="SELECT MIN(".$parametro.") as mini,AVG(".$parametro.") as medi,MAX(".$parametro.") as maxi, DATE from MeshliumDB.pfc where ID = ".$ID." group by DATE(`pfc`.`DATE`) ORDER BY NUM DESC ";
	 $myArray = array();
    if ($resultado=mysqli_query($link,$consulta))
	{
        $tempArray = array();
	...
	/*$minimo = $fila_registro['min'];
	$medio = $fila_registro['med'];
	$maximo = $fila_registro['max'];
	$dia = $fila_registro['dia'];
	
	$grafica = array($minimo,$medio,$maximo,$dia);
...
//Grafica de datos de registros y uso 15 DIAS
...
if ($_GET['grafica'] == 2){
	...
	for($a=0;$a<14;$a++){
		...
		$dia = time()-($a*24*60*60); //Te resta un dia (2*24*60*60) te resta dos y //asi...
